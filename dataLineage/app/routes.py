...
def get_lineage():
    ...
    table_dict = {table['id']: table['name'] for table in tables}
    nodes = {}
    for entry in lineage:
        source_name = table_dict[entry['source_table_id']]
        target_name = table_dict[entry['target_table_id']]
        if source_name not in nodes:
            nodes[source_name] = {'id': source_name, 'name': source_name, 'children': []}
        if target_name not in nodes:
            nodes[target_name] = {'id': target_name, 'name': target_name, 'children': []}
        nodes[source_name]['children'].append(nodes[target_name])

    # Find the root nodes
    roots = set(nodes.keys())
    for node in nodes.values():
        for child in node['children']:
            if child['id'] in roots:
                roots.remove(child['id'])

    # Create the final data structure for Highcharts
    chart_data = [nodes[root] for root in roots]
    print(chart_data)
    chart_data = [
                [None, 'Proto Indo-European'],
